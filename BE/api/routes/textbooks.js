...
const roleGuard = require("../middlewares/roleGuard");
 ...
 // body: { title }
 // 생성자 = author
 // 생성자를 enrollments에 teacher로 자동 등록
...
// 내가 author인 교재 목록 (latest_version 포함)
...
// teacher만 가능
router.post("/:textbookId/versions", roleGuard(["teacher"]), async (req, res) => {
  const client = await pool.connect();
  try {
...
// 수강 등록(학생/교사) 되어 있으면 모두 열람 가능
router.get("/:textbookId/versions/:version/pages", roleGuard(["student","teacher"]), async (req, res) => {
  try {
...
// body: { page_number, content(json) }
// teacher만
router.post("/:textbookId/versions/:version/pages", roleGuard(["teacher"]), async (req, res) => {
  try {
    const { textbookId, version } = req.params;
