...
import StartClassModal from "../teacher/StartClassModal";
...
const TeacherSidebar = () => {
  ...
  // 강의 시작 눌렀을 떄 버튼 활성화 + 모달 열기
  const handleClickStartButton = () => {
    setActivatedBtn(3);
    setIsStartClassOpen(true);
  };

  // 수업 시작 확정
  const handleStartClass = () => {
    setIsStartClassOpen(false);
    navigate("/teacher/lecture"); // 수업 화면
  };
    ...
    <aside className="w-[287px] shrink-0 bg-white border-r border-[#E2E8F0]">
      ...
      <div className="sticky top-16 h-[calc(100vh-64px)] overflow-y-auto pt-[16px] px-[12px]">
        ...
        <SidebarBtn
          ...
          onClick={handleClickStartButton} // 강의 시작
        >
          강의 시작
        </SidebarBtn>
      ...
      <StartClassModal
        open={isStartClassOpen}
        onClose={() => setIsStartClassOpen(false)}
        onConfirm={handleStartClass}
      />
    </aside>
  );
};
