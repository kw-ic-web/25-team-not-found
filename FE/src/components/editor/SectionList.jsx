  ...
  return (
    <aside className="w-80 h-full bg-white flex flex-col">
      ...
      <div className="px-6 pb-7">
        <div className="text-xs font-semibold tracking-wider uppercase text-slate-500">페이지</div>
      </div>

      <div className="flex-1 overflow-y-auto px-4 space-y-1">
        {sections.map((s) => {
          const active = s.id === currentId;
          const quizzes = quizzesByPage[s.id] || [];
          return (
            <div key={s.id} className="space-y-0.5">
              <button
                ...
                className={[
                  ...
                  active ? "bg-sky-50 text-sky-600" : "hover:bg-slate-100 text-slate-800",
                ].join(" ")}
              >
              ...
              {quizzes.length > 0 && (
                <div className="pl-4 space-y-0.5">
                  {quizzes.map((quiz) => (
                    <div
                      key={quiz.quiz_id}
                      className="px-2 py-1.5 text-xs text-slate-600 hover:bg-slate-50 rounded-md"
                    >
                      ㄴ {quiz.title}
                    </div>
                  ))}
      ...
      <div className="mt-4 p-4 space-y-2">
        ...
        <button
          className="w-full py-2 rounded-md text-sm bg-sky-500 text-white hover:bg-sky-600"
          onClick={() => {
