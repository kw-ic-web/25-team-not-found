 ...
 * @returns {JSX.Element}
 */
const SearchBookItem = ({ title, subject = "", term = "", textbookId }) => {
  const navigate = useNavigate();

  return (
    <div className="flex flex-col w-[320px] h-[242px] rounded-[16px] border border-[#E2E8F0]">
      <div className="w-full h-[112px] bg-gray-200 bg-center bg-cover rounded-t-[16px]"></div>
      <div className="flex flex-col gap-[4px] flex-1 min-h-0 p-[16px]">
        ...
        <div className="flex items-center gap-[8px] pt-[10px]">
          ...
          <button
            ...
            onClick={() => navigate("/student/quiz", { state: { textbookId: textbookId } })}
          >
            퀴즈
          </button>
        </div>
